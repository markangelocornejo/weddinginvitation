...
export function MusicToggle() {
  ...
  const playNextTrack = async () => {
    const nextTrackIndex = (trackIndex + 1) % invitationData.musicPlaylist.length
    const nextTrack = invitationData.musicPlaylist[nextTrackIndex]
  ...
  return (
    <div className="relative flex flex-col items-end">
      ...
      {isToastVisible && !isUnavailable && (
        <div
          className="absolute bottom-full right-0 mb-3 w-[min(17rem,calc(100vw-2rem))] rounded-[0.9rem] border border-[#D5B892]/75 bg-[#FDF8F0]/95 px-4 py-3 text-left text-[#3B2A1A] shadow-[0_14px_34px_rgba(59,42,26,0.18)] backdrop-blur-sm"
          role="status"
        >
          <p className="text-[0.56rem] font-bold uppercase tracking-[0.18em] text-[#7A5A3A]">Now playing</p>
          <div className="mt-1.5 flex items-center justify-between gap-3">
            <p className="min-w-0 flex-1 truncate font-serif text-[1.05rem] font-semibold leading-tight text-[#3B2A1A]">
              {currentTrack.title}
            </p>
            <button
              className="shrink-0 rounded-full border border-[#D5B892] bg-[#F5EBDD] px-3 py-1.5 text-[0.58rem] font-bold uppercase tracking-[0.12em] text-[#6E4C35] transition-colors hover:bg-[#EADBC5] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#B8862F]"
              type="button"
              onClick={() => {
                void playNextTrack()
              }}
